...
urlpatterns = [
    ...
    re_path(
        r"^accounts/login/",
        view=first_time_setup_wrapper(LoginView.as_view()),
        name="login",
    ),
    ...
    re_path(
        r"^service/create/", view=ServiceCreateView.as_view(), name="create-service"
    ),
    re_path(
        r"^service/update/(?P<pk>\d+)/",
        view=ServiceUpdateView.as_view(),
        name="update-service",
    ),
    re_path(
        r"^service/delete/(?P<pk>\d+)/",
        view=ServiceDeleteView.as_view(),
        name="delete-service",
    ),
    re_path(r"^service/(?P<pk>\d+)/", view=ServiceDetailView.as_view(), name="service"),
    re_path(
        r"^service/acknowledge_alert/(?P<pk>\d+)/",
    ...
    re_path(
        r"^instance/create/", view=InstanceCreateView.as_view(), name="create-instance"
    ),
    re_path(
        r"^instance/update/(?P<pk>\d+)/",
        view=InstanceUpdateView.as_view(),
        name="update-instance",
    ),
    re_path(
        r"^instance/duplicate/(?P<pk>\d+)/",
        view=duplicate_instance,
        name="duplicate-instance",
    ),
    re_path(
        r"^instance/delete/(?P<pk>\d+)/",
        view=InstanceDeleteView.as_view(),
        name="delete-instance",
    ),
    re_path(
        r"^instance/(?P<pk>\d+)/", view=InstanceDetailView.as_view(), name="instance"
    ),
    ...
    re_path(
        r"^check/delete/(?P<pk>\d+)/",
        view=StatusCheckDeleteView.as_view(),
        name="delete-check",
    ),
    re_path(r"^check/(?P<pk>\d+)/", view=StatusCheckDetailView.as_view(), name="check"),
    re_path(
        r"^checks/report/$", view=StatusCheckReportView.as_view(), name="checks-report"
    ),
    re_path(
        r"^icmpcheck/create/",
        view=ICMPCheckCreateView.as_view(),
    ...
    re_path(
        r"^icmpcheck/duplicate/(?P<pk>\d+)/",
        view=duplicate_icmp_check,
        name="duplicate-icmp-check",
    ),
    re_path(
...
if settings.URL_PREFIX.strip("/"):
    urlpatterns = [path(r"^%s/" % settings.URL_PREFIX.strip("/"), include(urlpatterns))]
