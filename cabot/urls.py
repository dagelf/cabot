...
urlpatterns = [
    ...
    path(r'^(?P<path>favicon\.ico)$', serve, name='favicon',
         kwargs={'document_root': os.path.join(settings.STATIC_ROOT, 'arachnys/img')}),

    path(r'^$', view=home_authentication_switcher,
         name='dashboard'),
    path(r'^subscriptions/', view=subscriptions,
         name='subscriptions'),
    path(r'^accounts/login/', view=first_time_setup_wrapper(LoginView.as_view()), name='login'),
    path(r'^accounts/logout/', view=LogoutView.as_view(), name='logout'),
    path(r'^setup/', view=SetupView.as_view(), name='first_time_setup'),
    path(r'^accounts/password-reset/',
    ...
    path(r'^api/oncall', view=OnCallView.as_view(), name='oncall'),
    path(r'^docs/',
...
if settings.URL_PREFIX.strip('/'):
    urlpatterns = [
        path(r'^%s/' % settings.URL_PREFIX.strip('/'), include(urlpatterns))
    ]
