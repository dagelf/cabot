...
class SymmetricalForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if instance.pk:
            for field in self.symmetrical_fields:
                symmetrical_field = getattr(instance, field)
                for symmetrical_object in self.cleaned_data[field]:
                    symmetrical_field.add(symmetrical_object)
            self.save_m2m()
        return instance
        ...
        widgets.update(
            {
                ...
                "metric": forms.TextInput(
                    attrs={"style": "width: 100%", "placeholder": "graphite metric key"}
                ),
...
class InstanceForm(SymmetricalForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "status_checks": forms.SelectMultiple(
                attrs={
                    "data-rel": "chosen",
                    "style": "width: 70%",
                }
            ),
            "service_set": forms.SelectMultiple(
                attrs={
                    "data-rel": "chosen",
                    "style": "width: 70%",
                }
            ),
            "alerts": forms.SelectMultiple(
                attrs={
                    "data-rel": "chosen",
                    "style": "width: 70%",
                }
            ),
            "users_to_notify": forms.SelectMultiple(
                attrs={
                    "data-rel": "chosen",
                    "style": "width: 70%",
                }
...
class ServiceForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "instances": forms.SelectMultiple(
                attrs={
                    "data-rel": "chosen",
                    "style": "width: 70%",
                }
...
class StatusCheckReportForm(forms.Form):
    service = forms.ModelChoiceField(
        queryset=Service.objects.all(), widget=forms.HiddenInput
    )
    ...
    date_from = forms.DateField(
        label="From", widget=forms.DateInput(attrs={"class": "datepicker"})
    )
    date_to = forms.DateField(
        label="To", widget=forms.DateInput(attrs={"class": "datepicker"})
    )
...
class StatusCheckListView(LoginRequiredMixin, CommonListView):
    ...
    def render_to_response(self, context, *args, **kwargs):
        ...
        if context is None:
            context = {}
        context["checks"] = (
            StatusCheck.objects.all()
            .order_by("name")
            .prefetch_related("service_set", "instance_set")
        )
        return super(StatusCheckListView, self).render_to_response(
            context, *args, **kwargs
        )
...
class StatusCheckDetailView(LoginRequiredMixin, CommonDetailView):
    ...
    def render_to_response(self, context, *args, **kwargs):
        ...
        return super(StatusCheckDetailView, self).render_to_response(
            context, *args, **kwargs
        )
...
class UserProfileUpdateView(LoginRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        return HttpResponseRedirect(
            reverse("update-alert-user-data", args=(self.kwargs["pk"], u"General"))
        )
    ...
    def post(self, request, pk, alerttype):
        ...
        if success:
            messages.add_message(
                request, messages.SUCCESS, "Updated Successfully", extra_tags="success"
            )
        else:
            messages.add_message(
                request, messages.ERROR, "Error Updating Profile", extra_tags="danger"
            )

        return HttpResponseRedirect(
            reverse("update-alert-user-data", args=(self.kwargs["pk"], alerttype))
        )
...
class PluginSettingsView(LoginRequiredMixin, View):
    ...
    def get(self, request, plugin_name):
        if plugin_name == u"global":
            ...
            alert_test_form = AlertTestPluginForm(initial={"alert_plugin": plugin})
    ...
    def post(self, request, plugin_name):
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.ERROR, "Error Updating Plugin", extra_tags="danger"
            )
...
class AlertTestPluginForm(AlertTestForm):
    ...
    alert_plugin = forms.ModelChoiceField(
        queryset=AlertPlugin.objects.filter(enabled=True), widget=forms.HiddenInput
    )
...
class AlertTestView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            self.trigger_alert_to_user(
                service, request.user, data["old_status"], data["new_status"]
            )
...
class GeneralSettingsForm(forms.Form):
    ...
    email_address = forms.CharField(
        label="Email Address", max_length=75, required=False
    )  # We use 75 and not the 254 because Django 1.6.8 only supports
...
class ServicePublicListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context[self.context_object_name] = (
            Service.objects.filter(is_public=True, alerts_enabled=True)
            .order_by(Lower("name"))
            .prefetch_related("status_checks")
        )
        return context
...
class InstanceDetailView(LoginRequiredMixin, CommonDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["report_form"] = StatusCheckReportForm(
            initial={
...
class InstanceCreateView(LoginRequiredMixin, CommonCreateView):
    ...
    def form_valid(self, form):
        ...
        if (
            self.object.status_checks.filter(
                polymorphic_ctype__model="icmpstatuscheck"
            ).count()
            == 0
        ):
...
class ShiftListView(LoginRequiredMixin, CommonListView):
    ...
    def get_queryset(self):
        return Shift.objects.filter(
            end__gt=datetime.utcnow().replace(tzinfo=utc), deleted=False
        ).order_by("start")
